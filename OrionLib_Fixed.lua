...
local OrionLib = {
	...
	Themes = {
		Default = {
			Main = Color3.fromRGB(5, 4, 56),
			Second = Color3.fromRGB(9, 7, 70),
			Stroke = Color3.fromRGB(34, 34, 62),
			Divider = Color3.fromRGB(34, 34, 62),
...
-- ▼ Filesystem fallbacks for weak executors
local FS = {
...
-- ▼ ScreenGui setup with executor fallbacks
...
local function SafeParent(gui)
	...
	elseif gethui then
		...
		if not ok or not gui.Parent then
			gui.Parent = LocalPlayer:FindFirstChildOfClass("PlayerGui") or LocalPlayer.PlayerGui
		end
	end
end
...
local function CleanDuplicates()
	...
	pcall(function() table.insert(parents, game.CoreGui) end)
	pcall(function() table.insert(parents, LocalPlayer.PlayerGui) end)
...
-- ▼ Toggle button
OrionLib._ToggleBtn = nil
OrionLib._MainWindow = nil
OrionLib._UIHidden = false

function OrionLib:CreateToggleButton(opts)
	opts = opts or {}
	local btnText = opts.Text or "UI"
	local btnSize = opts.Size or UDim2.new(0, 50, 0, 50)
	local btnPos  = opts.Position or UDim2.new(0, 20, 0.5, -25)

	if OrionLib._ToggleBtn then
		OrionLib._ToggleBtn:Destroy()
		OrionLib._ToggleBtn = nil
	end

	local Holder = Instance.new("Frame")
	Holder.Name = "Orion_ToggleButtonHolder"
	Holder.Size = btnSize
	Holder.Position = btnPos
	...
	AddDraggingFunctionality(btn, Holder)

	btn.MouseButton1Click:Connect(function()
		OrionLib:ToggleUI()
	end)

	OrionLib._ToggleBtn = Holder
	return Holder
end

function OrionLib:ToggleUI(force)
	if not OrionLib._MainWindow then return end
	local newState
	if force == nil then
		newState = not OrionLib._MainWindow.Visible
	else
		newState = force == true
	end
	OrionLib._MainWindow.Visible = newState
	OrionLib._UIHidden = not newState
end
...
local function SaveCfg(Name)
	...
	pcall(function()
		FS.writefile(OrionLib.Folder .. "/" .. Name .. ".txt", tostring(HttpService:JSONEncode(Data)))
	end)
end
...
-- ▼ Element creators
-- Active = false on all Frames so they don't swallow input
-- Active = true + Selectable = false on all Buttons
...
-- ▼ KEY FIX: Active = true, Selectable = false on every Button
...
OrionLib._NotifScale = Instance.new("UIScale")
OrionLib._NotifScale.Scale = 1
OrionLib._NotifScale.Parent = NotificationHolder
...
function OrionLib:MakeNotification(NotificationConfig)
	task.spawn(function()
		...
		local NotificationFrame = SetChildren(
			SetProps(MakeElement("RoundFrame", Color3.fromRGB(25, 25, 25), 0, 10), {
...
function OrionLib:Init()
	if OrionLib.SaveCfg then
		pcall(function()
			if FS.isfile(OrionLib.Folder .. "/" .. GameId .. ".txt") then
				LoadCfg(FS.readfile(OrionLib.Folder .. "/" .. GameId .. ".txt"))
				OrionLib:MakeNotification({
					Name    = "Configuration",
					Content = "Auto-loaded configuration for the game " .. GameId .. ".",
					Time    = 5
				})
			end
		end)
...
function OrionLib:GetEffectiveScale()
	return (self._WindowScale and self._WindowScale.Scale) or 1
end
...
function OrionLib:MakeWindow(WindowConfig)
	...
	WindowConfig.UIScale      = WindowConfig.UIScale      or 1
	...
	if WindowConfig.SaveConfig then
		pcall(function()
			if not FS.isfolder(WindowConfig.ConfigFolder) then
				FS.makefolder(WindowConfig.ConfigFolder)
			end
		end)
	end
	...
	AddConnection(TabHolder.UIListLayout:GetPropertyChangedSignal("AbsoluteContentSize"), function()
		OrionLib:_SetScrollCanvas(TabHolder, 16)
	end)

	-- ▼ Close and Minimize buttons: ZIndex high, Active true
	...
	-- ▼ DragPoint: ZIndex LOW and Active = false so it never blocks buttons
		...
		SetChildren(
			SetProps(MakeElement("RoundFrame", Color3.fromRGB(255, 255, 255), 0, 10), {
				...
				SetChildren(SetProps(MakeElement("TFrame"), {
					...
					AddThemeObject(SetProps(MakeElement("Label", LocalPlayer.DisplayName,
						WindowConfig.HidePremium and 14 or 13
					), {
	...
	-- ▼ Button container for close/minimize — ZIndex above DragPoint
	local TopBarBtnContainer = AddThemeObject(
		SetChildren(
			SetProps(MakeElement("RoundFrame", Color3.fromRGB(255, 255, 255), 0, 7), {
				Size     = UDim2.new(0, 70, 0, 30),
				Position = UDim2.new(1, -90, 0, 10),
				ZIndex   = 4   -- above DragPoint (1) and topbar (2)
			}), {
				AddThemeObject(MakeElement("Stroke"), "Stroke"),
				AddThemeObject(SetProps(MakeElement("Frame"), {
					Size     = UDim2.new(0, 1, 1, 0),
	...
	OrionLib._MainWindow = MainWindow

	if WindowConfig.ToggleButton == nil then
		WindowConfig.ToggleButton = true
	end
	if WindowConfig.ToggleButton then
	...
	OrionLib._WindowScale = Instance.new("UIScale")
	OrionLib._WindowScale.Scale = WindowConfig.UIScale
	OrionLib._WindowScale.Parent = MainWindow
	...
	AddConnection(CloseBtn.MouseButton1Up, function()
		OrionLib:ToggleUI(false)
		OrionLib._UIHidden = true
		UIHidden = true
		OrionLib:MakeNotification({
			Name    = "Interface Hidden",
			Content = "Tap RightShift or the UI button to reopen",
			Time    = 5
		})
	...
	function TabFunction:MakeTab(TabConfig)
		...
		AddConnection(Container.UIListLayout:GetPropertyChangedSignal("AbsoluteContentSize"), function()
			OrionLib:_SetScrollCanvas(Container, 30)
		end)
		...
		local function GetElements(ItemParent)
			...
			function ElementFunction:AddDropdown(DropdownConfig)
				...
				local DropdownContainer = AddThemeObject(
					SetProps(
						SetChildren(MakeElement("ScrollFrame", Color3.fromRGB(40, 40, 40), 4), {
							DropdownList
						}), {
							Parent          = ItemParent,
				...
				AddConnection(DropdownList:GetPropertyChangedSignal("AbsoluteContentSize"), function()
					OrionLib:_SetScrollCanvas(DropdownContainer)
				end)
				...
				function Dropdown:Set(Value)
					...
					TweenService:Create(Dropdown.Buttons[Value], TweenInfo.new(.15, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
						BackgroundTransparency = 0
					}):Play()
					TweenService:Create(Dropdown.Buttons[Value].Title, TweenInfo.new(.15, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
						TextTransparency = 0
					}):Play()
				...
				Color.InputBegan:Connect(function(input)
					if input.UserInputType == Enum.UserInputType.MouseButton1 or
						...
						ColorInput = RunService.RenderStepped:Connect(function()
							local inputPosition = UserInputService:GetLastInputType() == Enum.UserInputType.Touch and
								input.Position or UserInputService:GetMouseLocation()
							local ColorX = math.clamp((inputPosition.X - Color.AbsolutePosition.X) / Color.AbsoluteSize.X, 0, 1)
							local ColorY = math.clamp((inputPosition.Y - Color.AbsolutePosition.Y) / Color.AbsoluteSize.Y, 0, 1)
...
function OrionLib:SetScale(scale)
	local s = math.clamp(tonumber(scale) or 1, 0.5, 2)
	if self._WindowScale then self._WindowScale.Scale = s end
	if self._NotifScale  then self._NotifScale.Scale  = s end
	for _, obj in ipairs(self.ThemeObjects.Divider or {}) do
		if obj:IsA("ScrollingFrame") then
			OrionLib:_SetScrollCanvas(obj, OrionLib._ScrollPads and OrionLib._ScrollPads[obj] or 0)
		end
	end
end

function OrionLib:GetScale()
	return (self._WindowScale and self._WindowScale.Scale) or 1
end

OrionLib._ScrollPads = OrionLib._ScrollPads or {}

function OrionLib:_SetScrollCanvas(sf, extra)
	extra = extra or 0
	OrionLib._ScrollPads[sf] = extra
